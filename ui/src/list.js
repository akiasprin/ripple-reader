    ...
    return `<div class="paper-card" data-id="${this.escape(p.id)}">
      ...
      <div class="paper-meta">
        ...
        <div class="meta-left">
          ...
          ${p.is_analyzing ? `<span class="meta-link" style="cursor:pointer" onclick="app.cancelInsight('${this.escape(p.id)}')">abortLLM</span>` : (p.is_reviewing ? `<span class="meta-link" style="cursor:pointer" onclick="app.cancelReviewTask('${this.escape(p.id)}')">abortLLM</span>` : (p.has_insight ? `<a href="/insight/${encodeURIComponent(p.source_type || 'arxiv')}/${p.id.split('/').map(encodeURIComponent).join('/')}" class="meta-link" onclick="if (!event.metaKey && !event.ctrlKey) { event.preventDefault(); app.showInsightPage('${this.escape(p.id)}'); }">LLM</a>` : (this.authenticated ? `<span class="meta-link" style="cursor:pointer" onclick="app.toggleInsight('${this.escape(p.id)}', event)">reqLLM</span>` : '')))}
        </div>
