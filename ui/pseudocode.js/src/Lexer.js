...
/* Math pattern
    ...
    Also supports display math: $$...$$ and \[...\].
*/
var mathPattern = {
    exec: function (str) {
        var delimiters = [
        ...
        for (var di = 0; di < delimiters.length; di++) {
            ...
            while (endPos < totalLen) {
                ...
                var fullMatch = str.slice(0, endPos + pos + endDel.length);
                var innerText = str.slice(startDel.length, endPos + pos);
                // Include delimiters in the token text for display math so
                // Renderer can distinguish inline vs display.
                var res = [fullMatch,
                    isDisplay ? fullMatch : innerText];
                return res;
            }
        }
