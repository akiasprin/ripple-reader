...
Parser.prototype.parse = function () {
    ...
    while (true) {
        var envInfo = this._acceptEnvironment();
        if (envInfo === null) break;

        var envNode;
        var envName = envInfo.name;
        var envOption = envInfo.option;
        if (envName === 'algorithm')
            envNode = this._parseAlgorithmInner();
        else if (envName === 'algorithmic')
            envNode = this._parseAlgorithmicInner(envOption);
        else
...
Parser.prototype._acceptEnvironment = function () {
    ...
    // Accept optional argument, e.g. \begin{algorithmic}[1]
    // The entire [1] is tokenized as a single ordinary atom.
    var option = null;
    var nextAtom = lexer._nextAtom;
    if (nextAtom && nextAtom.type === 'ordinary' && nextAtom.text.startsWith('[')) {
        var text = lexer.accept('ordinary');
        option = text.slice(1);
        if (option.endsWith(']')) {
    ...
    return { name: envName, option: option };
};
...
Parser.prototype._parseAlgorithmInner = function () {
    ...
    while (true) {
        var envInfo = this._acceptEnvironment();
        if (envInfo !== null) {
            if (envInfo.name !== 'algorithmic')
                throw new ParseError(`Unexpected environment ${envInfo.name}`);
            var algmicNode = this._parseAlgorithmicInner(envInfo.option);
            this._closeEnvironment(envInfo.name);
        ...
        // Handle loose statements/font commands between algorithmic end and algorithm end
        // (e.g. \textbf{Input:}, \textit{Note:})
        var looseStmt = this._parseStatement(STATEMENTS);
        if (looseStmt) {
            algNode.addChild(looseStmt);
            continue;
        }
...
Parser.prototype._parseAlgorithmicInner = function (option) {
    var algmicNode = new ParseNode('algorithmic');
    algmicNode.value = { option: option };
    var node;
    while (true) {
...
Parser.prototype._parseBlock = function () {
    ...
    while (true) {
        ...
        if (this._lexer.accept('func', ['quad', 'qquad'])) {
            continue;
        }
