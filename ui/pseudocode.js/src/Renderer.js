    ...
    while ((node = this._nodes.shift()) !== undefined) {
        ...
        switch (type) {
            ...
            case 'math':
                ...
                // Detect display math by preserved delimiters ($$...$$ or \[...\]).
                var isDisplay = false;
                var innerText = text;
                if (text.startsWith('$$') && text.endsWith('$$')) {
                    isDisplay = true;
                    innerText = text.slice(2, -2);
                } else if (text.startsWith('\\[') && text.endsWith('\\]')) {
                    isDisplay = true;
                    innerText = text.slice(2, -2);
                }

                if (backend.name === 'katex') {
                    this._html.putHTML(backend.driver.renderToString(
                        innerText, { displayMode: isDisplay }));
                }
                else if (backend.name === 'mathjax') {
                    if (backend.version < 3) {
                        // keep math text, typeset later
                        var wrapper = isDisplay ? '$$' : '$';
                        this._html.putText(`${wrapper}${innerText}${wrapper}`);
                    }
                    else {
                        // use synchronous conversion available in 3.x
                        this._html.putHTML(backend.driver.tex2chtml(
                            innerText, { display: isDisplay }).outerHTML);
                    }
                }
                else {
            ...
            case 'space-symbol':
                ...
                break;
            case 'hfill-symbol':
                // Collect remaining nodes and render them in a float:right span
                var remainingNodes = [];
                while ((node = this._nodes.shift()) !== undefined) {
                    remainingNodes.push(node);
                }
                if (remainingNodes.length > 0) {
                    this._html.putHTML('<span style="float: right; padding-left: 1em;">');
...
Renderer.prototype._buildTree = function (node) {
    ...
    switch (node.type) {
        ...
        case 'algorithm':
            ...
            break;
        case 'algorithmic':
            // An optional argument like \begin{algorithmic}[1] enables line numbering
            var savedLineNumber = this._options.lineNumber;
            if (node.value && node.value.option) {
                this._options.lineNumber = true;
            }
            ...
            this._buildTreeForAllChildren(node);
            this._endGroup();
            this._options.lineNumber = savedLineNumber;
            break;
        case 'block':
            // node: <block>
            // ==>
        ...
        case 'statement':
            ...
            // Also handles LaTeX font commands (\textbf, \textit, etc.)
            // that appear at the block level.
            ...
            this._newLine();
            if (displayStmtName !== undefined) {
                if (displayStmtName) this._typeKeyword(displayStmtName);
                textNode = node.children[0];
                this._buildTree(textNode);
            } else {
                // Treat as a font command (\textbf, \textit, etc.)
                // Apply the font style via TextStyle, falling back to
                // plain text if the command is not recognized.
                try {
                    var innerTextStyle = new TextStyle(this._globalTextStyle.fontSize());
                    innerTextStyle.updateByCommand(stmtName);
                    this._html.beginSpan(null, innerTextStyle.toCSS());
                    textNode = node.children[0];
                    this._buildTree(textNode);
                    this._html.endSpan();
